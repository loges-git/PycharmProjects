...
def generate_release_branch_name(context) -> str:
    ...
    prefix_map = {
        ...
        ReleaseType.FIX: "bugfix",
        ReleaseType.ROLLBACK: "hotfix"
    }
...
def render_release_flow(context: ReleaseContext) -> None:
    ...
    st.success(f"✅ Cluster selected: {context.cluster}")
    st.divider()
    ...
    st.success(f"✅ Release Type selected: {context.release_type.value}")
    st.divider()
